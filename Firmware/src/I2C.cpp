...
bool I2C::write(uint8_t device, uint8_t value, bool no_stop)
{
	return write(device, &value, 1, no_stop);
}

bool I2C::write(uint8_t device, const uint8_t* data, size_t size, bool no_stop)
{
	auto err = nrf_drv_twi_tx(&m_twi, device, data, size, no_stop);
	return err == NRF_SUCCESS;
}

bool I2C::read(uint8_t device, uint8_t* value)
{
	return read(device, value, 1);
}

bool I2C::read(uint8_t device, uint8_t* data, size_t size)
{
